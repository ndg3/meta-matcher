...
export default function FighterList({
	...
	name: string, res: {[key: string]: {[key: string]: number | string}}
}) {
	return (
		<>
			<h1>Matches for {parseName(name)}</h1>
			<ul>{Object.entries(res).slice(0, -2).map(([ fighter, counterpicks ], idx) => {
				return (
					<div key={`div:${idx}`}>
				...
				{Object.keys(res.warnings).length !== 0 ?
				 <>
					 ...
					 <ul>{Object.entries(res.warnings).map(([ n, v ]) =>
...
const parseName = (name: string) => name in characters ? getNameFromApi(name) : name;
